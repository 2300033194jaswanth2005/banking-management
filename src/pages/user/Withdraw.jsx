...
export default function Withdraw() {
  ...
  function handleSubmit(e) {
    e.preventDefault()
    const res = withdraw(currentUser.accNo, Number(amount))
    setMsg(res)
    if (res.ok) setAmount('')
  }
    ...
    <div className="p-6 max-w-md mx-auto">
      ...
      <div className="bg-white rounded-xl shadow p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {msg && <p className={`text-sm ${msg.ok ? 'text-green-600' : 'text-red-500'}`}>{msg.ok ? '✅ Withdrawal successful!' : msg.msg}</p>}
          <button type="submit" className="w-full bg-red-600 text-white py-2 rounded-lg font-semibold hover:bg-red-700 transition">Withdraw</button>
        </form>
      </div>
    </div>
  )
