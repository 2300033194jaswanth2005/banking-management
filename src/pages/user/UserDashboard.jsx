...
export default function UserDashboard() {
  ...
  const recent = getMyTransactions(currentUser.accNo).slice(0, 5)

  return (
    <div className="p-6 max-w-4xl mx-auto">
      <h2 className="text-2xl font-bold text-gray-800 mb-1">Welcome, {currentUser.name} 👋</h2>
      <p className="text-gray-500 text-sm mb-6">Account No: <span className="font-semibold text-blue-700">{currentUser.accNo}</span></p>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <Card title="Current Balance" value={`₹ ${Number(currentUser.balance).toFixed(2)}`} color="blue" />
        <Card title="Total Transactions" value={getMyTransactions(currentUser.accNo).length} color="green" />
        ...
        {recent.length === 0 ? (
          ...
          <table className="w-full text-sm">
            <thead><tr className="text-left text-gray-500 border-b">
              <th className="pb-2">Type</th><th className="pb-2">Amount</th><th className="pb-2">Note</th><th className="pb-2">Date</th>
            </tr></thead>
            <tbody>
              {recent.map((t, i) => (
