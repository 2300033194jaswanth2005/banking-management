...
export default function Transfer() {
  const { currentUser, transfer, getAllUsers } = useAuth()
    ...
    <div className="p-6 max-w-md mx-auto">
      ...
      <div className="bg-white rounded-xl shadow p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <input
            name="toAccNo" type="number" placeholder="Recipient Account No"
            value={form.toAccNo} onChange={set}
          ...
          {msg && (
            <p className={`text-sm px-3 py-2 rounded-lg ${msg.ok ? 'text-green-600 bg-green-50' : 'text-red-500 bg-red-50'}`}>
              {msg.ok ? '✅ Transfer successful!' : msg.msg}
            </p>
          )}
